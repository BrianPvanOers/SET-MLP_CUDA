...
def backpropagation_updates_numpy_CUDA(a, delta, rows, cols, out):
    i = cuda.grid(1)
    if i < out.shape[0]:
        s = 0
        for j in range(a.shape[0]):
...
def load_data(filename, test_size, folder="imputed_data"):
    ...
    if dataset == "spam":
        ...
        # Get the features from the imputed data
        imputed_data = pd.read_csv(os.path.join(folder, filename), delimiter=";")
        x = imputed_data.iloc[:, :-1]
        # Get the labels from the reference data
        data = pd.read_csv("data/winequality-red.csv", delimiter=";")
        y = data.iloc[:, -1]
    else:
        raise ValueError("filename must be 'spam', 'letter' or 'winequality-red'")
    ...
    # Convert to np array
    x, y = x.to_numpy(dtype='float64'), y.to_numpy(dtype='float64')

    # Fix y dimensions for 1d array
    if len(y.shape) == 1:
...
if __name__ == "__main__":
    # Get all the files from the imputed data folder
    folder = "imputed_data"  # 'data' for reference data or 'imputed_data' for imputed data
    ...
    for filename in files:
        ...
        for i in range(runs):
            ...
            x_train, y_train, x_test, y_test = load_data(filename, test_size, folder=folder)
            ...
            np.random.seed(0)  # set to 0 for consistency

            # create SET-MLP (MLP with adaptive sparse connectivity trained with Sparse Evolutionary Training)
            set_mlp = SET_MLP((x_train.shape[1], no_hidden_neurons_layer, no_hidden_neurons_layer,
                               no_hidden_neurons_layer, y_train.shape[1]), (Relu, Relu, Relu, Softmax), epsilon=epsilon)
            ...
            set_mlp.fit(x_train, y_train, x_test, y_test, loss=CrossEntropy, epochs=no_training_epochs,
                        ...
                        zeta=zeta, dropoutrate=dropout_rate, testing=True, save_filename="results/set_mlp_" + filename,
                        monitor=True)
